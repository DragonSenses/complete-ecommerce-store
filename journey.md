...
4. Declare an `event` variable which is a type of `Stripe.Event`. Then open a `try..catch` block, where we assign the result of `stripe.webhooks.constructEvent` to event. Pass in `body`, `signature`, and `process.env.STRIPE_WEBHOOK_SECRET`. Meanwhile in the catch, return a `NextResponse` status 400 while printing the error message.
